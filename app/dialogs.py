...
from data.data import (
    obtenir_commandes_perso, supprimer_commande, ajouter_commande,
    ressource_path, obtenir_categories, ajouter_categorie,
    F_CONFIG, F_SCORES,
)
from app.theme_utils import centrer_fenetre, basculer_theme, reinitialiser_application


# ── Helpers ───────────────────────────────────────────────────────────────────

def _bind_mousewheel(widget, canvas):
    """Scroll molette cross-platform (Windows + Linux)."""
    def _scroll(event):
        if event.delta:
...
def demander_profil(self):
    ...
    Label(fen, text="BIENVENUE", font=("Segoe UI", 18, "bold"),
          bg="#F9FAFB", fg="#312E81", pady=25).pack()
    ...
    def valider():
        ...
        if p and n:
            ...
            # Écriture dans F_CONFIG (chemin runtime correct pour l'exe)
    ...
    Button(fen, text="CRÉER MON PROFIL 🐧", bg="#312E81", fg="white",
           font=("Segoe UI", 10, "bold"), command=valider, pady=15, relief="flat"
...
def ouvrir_aide(self):
    ...
    tk.Button(fen, text="J'AI COMPRIS 🐧", command=fermer,
              bg=self.c_accent, fg="white", font=("Segoe UI", 11, "bold"),
              pady=15, relief="flat").pack(side="bottom", fill="x")

    tk.Label(scroll_frame, text="🐧 GUIDE COMPLET LINUX PRO",
             font=("Segoe UI", 16, "bold"), bg=self.c_card, fg=self.c_accent).pack(pady=20, padx=40)
    ...
    ajouter_section("👤 GESTION DU PROFIL",
        "Au premier lancement, le logiciel vous demande votre Nom et Prénom. "
        "Votre identifiant Linux (ex : jean.dupont) est généré automatiquement pour simuler "
        "le terminal. Vous pouvez réinitialiser ce profil dans les Paramètres.")
    ajouter_section("🌓 THÈMES ET APPARENCE",
        "Deux modes disponibles : Clair et Sombre. Basculez via le bouton 'PARAMÈTRES'. "
        "Votre choix est sauvegardé automatiquement dans config.json.")
    ajouter_section("🔍 NAVIGATION ET RECHERCHE",
        "La barre de recherche filtre instantanément les commandes par nom ou catégorie. "
        "Cliquez sur les en-têtes du tableau pour trier la liste (↑ ou ↓).")
    ajouter_section("🎯 FONCTIONNEMENT DU QUIZ",
        "1. Choisissez un défi (5 à 20 questions).\n"
        "2. Une description s'affiche — trouvez la commande correspondante.\n"
        "3. À la fin : score + bilan scrollable de toutes les réponses.\n"
        "4. Recommencez ou quittez à tout moment.")
    ajouter_section("➕ AJOUTER UNE COMMANDE",
        "1. Sélectionnez une catégorie existante (badge) ou créez-en une nouvelle.\n"
        "2. Remplissez : Nom, Description, Exemple.\n"
        "La commande est sauvegardée dans commandes_personnelles.json.")
    ajouter_section("🎨 NOUVELLE CATÉGORIE",
        "Cliquez sur 'VALIDER' pour confirmer le nom, puis choisissez une icône emoji "
        "et une couleur de fond. La catégorie est ajoutée à categories.json.")
    ajouter_section("🗑️ SUPPRESSION",
        "Seules vos commandes personnelles (créées via AJOUTER) peuvent être supprimées. "
        "Les commandes système (commandes.json) sont protégées en lecture seule.")
...
def ouvrir_parametres(self):
    ...
    Label(fen, text="⚙️ RÉGLAGES", font=("Segoe UI", 13, "bold"),
          bg=self.c_card, fg=self.c_fg).pack(pady=(0, 25))

    txt = "PASSER AU MODE CLAIR ☀️" if self.mode_sombre else "PASSER AU MODE SOMBRE 🌙"
    Button(fen, text=txt, font=("Segoe UI", 10, "bold"), bg=self.c_accent, fg="white",
           pady=12, relief="flat",
           command=lambda: [basculer_theme(self), fen.destroy()]).pack(fill="x", pady=8)

    Button(fen, text="RÉINITIALISER LE PROFIL 🔄", font=("Segoe UI", 10, "bold"),
           bg="#F59E0B", fg="white", pady=12, relief="flat",
...
def ouvrir_statistiques(self):
    # Utilise F_SCORES (runtime_path) au lieu de "scores.json" brut
    if not os.path.exists(F_SCORES) or os.stat(F_SCORES).st_size == 0:
    ...
    Label(fen, text="HISTORIQUE DES QUIZ", font=("Segoe UI", 14, "bold"),
          bg=self.c_card, fg=self.c_accent).pack(pady=15)

    cadre_tree = tk.Frame(fen, bg=self.c_card)
    cadre_tree.pack(fill="both", expand=True, padx=30, pady=(0, 20))

    tree = ttk.Treeview(cadre_tree, columns=("date", "type", "score"), show="headings", height=15)
    ...
    scrollbar = ttk.Scrollbar(cadre_tree, orient="vertical", command=tree.yview)
    ...
    Button(fen, text="FERMER", bg=self.c_accent, fg="white",
           font=("Segoe UI", 10, "bold"), pady=10, relief="flat",
           command=fen.destroy).pack(fill="x", padx=30, pady=(0, 15))
...
def ouvrir_ajout(self):
    ...
    couleurs_utilisees = [d["couleur"] for d in categories_data.values()]
    couleurs_dispo     = [c for c in couleurs_potentielles if c not in couleurs_utilisees] or couleurs_potentielles
    ...
    ent_saisie_libre   = tk.Entry(cadre_saisie_cat, font=("Segoe UI", 10),
                                   bg=self.c_bg, fg=self.c_fg, relief="solid", borderwidth=1)
    cadre_custom_icon  = tk.Frame(fen, bg=self.c_card)
    cadre_custom_coul  = tk.Frame(fen, bg=self.c_card)
    ent_cat_finale     = tk.Entry(fen, font=("Segoe UI", 10, "bold"),
                                   bg="#F3F4F6", fg=self.c_accent, relief="flat", justify="center")

    def actualiser_vue():
        cat = self.var_cat.get()
        txt = "Sélectionnez une catégorie..." if not cat else \
              f"{self.icones.get(cat, self.icone_choisie_finale or '📦')} {cat}"
    ...
    def valider_nouvelle_cat():
        ...
        if s:
            ...
            cadre_custom_coul.pack(fill="x", pady=5, after=cadre_custom_icon)
            actualiser_vue()
        else:
            messagebox.showwarning("Attention", "Saisissez un nom de catégorie.", parent=fen)

    # ── Section 1 : catégorie ─────────────────────────────────
    tk.Label(fen, text="1. CHOIX DE LA CATÉGORIE", font=("Segoe UI", 9, "bold"),
             bg=self.c_card, fg=self.c_accent).pack(anchor="w")

    cadre_badges = tk.Frame(fen, bg=self.c_card)
    cadre_badges.pack(fill="x", pady=5)
    row, col = 0, 0
    for c in self.categories:
        tk.Button(cadre_badges, text=c, font=("Segoe UI", 8),
                  command=lambda x=c: selectionner_cat_existante(x),
                  bg=self.c_bg, fg=self.c_fg, relief="flat"
    ...
    tk.Label(fen, text="Ou créer une nouvelle catégorie :", font=("Segoe UI", 8),
             bg=self.c_card, fg="#6B7280").pack(anchor="w", pady=(10, 0))
    ...
    # Grille icônes
    tk.Label(cadre_custom_icon, text="Choisir une icône :", font=("Segoe UI", 8, "italic"),
             bg=self.c_card, fg=self.c_accent).pack(anchor="w")
    ...
    # Grille couleurs
    tk.Label(cadre_custom_coul, text="Choisir une couleur :", font=("Segoe UI", 8, "italic"),
             bg=self.c_card, fg=self.c_accent).pack(anchor="w")
    zone_couleurs = tk.Frame(cadre_custom_coul, bg=self.c_card)
    zone_couleurs.pack(pady=2)
    r_col, c_col = 0, 0
    for cl in couleurs_dispo:
        tk.Button(zone_couleurs, bg=cl, width=5, height=2,
                  command=lambda c=cl: setattr(self, "couleur_choisie_finale", c),
    ...
    # ── Section 2 : détails ───────────────────────────────────
    tk.Label(fen, text="2. DÉTAILS DE LA COMMANDE", font=("Segoe UI", 9, "bold"),
             bg=self.c_card, fg=self.c_accent).pack(anchor="w", pady=(5, 5))
    ...
    for lib, cl in [("Nom", "n"), ("Description", "d"), ("Exemple", "e")]:
        ...
        ch[cl] = tk.Entry(fen, font=("Segoe UI", 10), bg=self.c_bg, fg=self.c_fg,
    ...
    def sau():
        ...
        couleur_finale = (couleur if cat not in self.categories
                          else obtenir_categories().get(cat, {}).get("couleur", couleur))

        if cat not in self.categories:
            ajouter_categorie(cat, icone_finale, couleur_finale)
            # Mettre à jour la liste locale des catégories connues
            self.categories.append(cat)
            self.icones[cat] = icone_finale
        ...
        if ajouter_commande(nom, des, exe, cat, icone_finale, couleur_finale):
            ...
            self.tableau.trier("cmd")   # ← corrigé (plus self.trier_colonne)
            fen.destroy()
        else:
            messagebox.showerror("Erreur ❌", "Erreur lors de l'enregistrement.", parent=fen)

    tk.Button(fen, text="ENREGISTRER LA COMMANDE 🐧", bg="#10B981", fg="white",
              font=("Segoe UI", 10, "bold"), pady=12, command=sau, relief="flat"
...
def ouvrir_suppression(self):
    ...
    tk.Label(fen, text="MES COMMANDES PERSO", font=("Segoe UI", 12, "bold"),
             bg=self.c_card, fg="#EF4444").pack(pady=(0, 15))
    ...
    for nom in sorted(commandes_perso.keys()):
        ...
        tk.Label(f, text=f"• {nom.upper()}", font=("Segoe UI", 10),
                 bg=self.c_card, fg=self.c_fg).pack(side="left")
